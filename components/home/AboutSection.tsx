      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="grid lg:grid-cols-2 gap-20 items-center">
          <div className="order-2 lg:order-1 relative animate-fade-in-up">
            <div className="grid grid-cols-2 gap-6 relative">
               ...
               <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 z-20 bg-white p-6 rounded-3xl shadow-2xl border-8 border-slate-50 rotate-6 hidden md:block">
                  <p className="text-4xl font-black text-primary">{content?.statsValue || "12+"}</p>
                  <p className="text-xs font-black text-slate-400 uppercase tracking-widest leading-none mt-1">{content?.statsLabel || "Years of Excellence"}</p>
               </div>

              <div className="space-y-6">
                <div className="h-72 rounded-[40px] overflow-hidden shadow-2xl relative group">
                   <Image src={content?.image1 || defaultImages[0]} alt="Activity" fill className="object-cover transition-transform duration-700 group-hover:scale-110" />
                   <div className="absolute inset-0 bg-primary/20 opacity-0 group-hover:opacity-100 transition-opacity"></div>
                </div>
                <div className="h-48 rounded-[40px] overflow-hidden shadow-2xl relative group">
                   <Image src={content?.image2 || defaultImages[1]} alt="Activity" fill className="object-cover transition-transform duration-700 group-hover:scale-110" />
                   <div className="absolute inset-0 bg-primary/20 opacity-0 group-hover:opacity-100 transition-opacity"></div>
                </div>
              </div>
              <div className="space-y-6 pt-12">
                <div className="h-48 rounded-[40px] overflow-hidden shadow-2xl relative group">
                   <Image src={content?.image3 || defaultImages[2]} alt="Activity" fill className="object-cover transition-transform duration-700 group-hover:scale-110" />
                   <div className="absolute inset-0 bg-primary/20 opacity-0 group-hover:opacity-100 transition-opacity"></div>
                </div>
                <div className="h-72 rounded-[40px] overflow-hidden shadow-2xl relative group">
                   <Image src={content?.image4 || defaultImages[3]} alt="Activity" fill className="object-cover transition-transform duration-700 group-hover:scale-110" />
                   <div className="absolute inset-0 bg-primary/20 opacity-0 group-hover:opacity-100 transition-opacity"></div>
                </div>
              </div>
            </div>
          ...
          <div className="order-1 lg:order-2 animate-fade-in-up" style={{ animationDelay: '0.2s' }}>
            <span className="text-primary font-black text-sm tracking-[0.2em] uppercase mb-4 inline-block">{content?.badge || "Since 2012"}</span>
            <h3 className="text-3xl lg:text-4xl font-black text-slate-900 mb-6 leading-tight tracking-tight">
              {content?.title || "Empowering Future Generations Through Cognitive Growth"}
            </h3>
            <div className="space-y-6 text-slate-600 leading-relaxed text-base font-medium">
              <p>
                {content?.description || "Orphosys Foundation is one of the best Kolkata based Child Brain Development Institution, dealing with several scientifically proven technologies of identifying child inner capabilities and polishing them to reach their heights."}
              </p>
              ...
              <p>
                {content?.description2 || "We are the fastest growing brand with multiple training programs. Our vision is to reach out to every household and contribute to the nation by developing young geniuses."}
              </p>
            </div>
